...
def shop(request,filterby="date",filterfield="CreateDate",showcount=12):
    if request.method == 'POST':
        ...
        if(filterby=="category"):
            ...
            products=Product.objects.annotate(minPrice=Min('variantproduct__mainPrice')).filter(minPrice__lte=filterfield[1],minPrice__gte=filterfield[0])
        elif(filterby=="size"):
            products=Product.objects.annotate(minPrice=Min('variantproduct__mainPrice')).filter(variantproduct__size=filterfield)
        ...
        sizes=VariantProduct.objects.values('size').order_by('size').distinct()
        print('\033[91m' +"print:  "+ str(sizes) + '\033[0m')
        
        context={'products': products , 'categories':categories,'pcount':pcount,'sizes':sizes}
...
def order(request):
    try:
        if(request.POST):
            #print('\033[91m' +"print:  "+ str(request.POST) + '\033[0m')
            print('\033[91m' +"print:  "+ str(request.user.first_name) + '\033[0m')
            address=request.POST['city']+'  '+request.POST['address']
            cartitems=CartItem.objects.filter(shopCart__user=request.user)
            informations="name :"+request.POST['fname']+" "+request.POST['lname']+"\n address"+address+ "\npostal code :"+request.POST['postalcode']+"\n mobilephone :"+request.POST['mobilephone']+" , phone :"+request.POST['phone']+"\nemail :"+request.POST['email']+"\n"
            order = Order.objects.create(user=request.user,createDate=datetime.now(),informations=informations,note=request.POST['note'])
            ...
            for cartitem in cartitems:
                OrderItem.objects.create(order=order ,variantProduct=cartitem.variantProduct,price=cartitem.variantProduct.discountPrice,status='o')
